...
def basic_model(pp_net: pp.pandapowerNet, rel: list, line_from: int, line_to: int, line_tol: float,
    ...
    # 支路功率越限
    FORMULATION = 0  # 0:广义约束  1:大M法  2 最大值形式
    ...
    if model.status == grb.GRB.Status.INFEASIBLE:
        ...
        for e in range(nSolution):
            ...
            fp.append(41 - sum([z[i].Xn for i in ij]))
        ...
        print(f'最高{np.max(np.array(fp))}阶故障')
...
if __name__ == "__main__":
    ...
    for i in range(len(ext_net.line)):
        ...
        tolerance = (ext_net.line['max_i_ka'].iloc[i] * net_vn) * np.sqrt(3)
        _, _, prob[i] = basic_model(ext_net, rel, f_bus, t_bus, tolerance, pool_num=10, pool_search_method=2)
    total_time = time.time() - st
    print(f'计算用时{total_time}')
    # exp_prob = np.expm1(prob)
    # total_prob = np.array(rel) + prob
